...
@IBDesignable
class BottomTopBorderView: UIView {
    
    var ok: CGFloat = 0.0
    
    @IBInspectable var borderBottomTopWidth:CGFloat {
        get { return self.ok }
        set {
            
            self.ok = newValue
            self.addViewBackedBorder(side: .top, thickness: self.ok, color: UIColor(white: 230.0 / 255.0, alpha: 1.0))
            self.addViewBackedBorder(side: .bottom, thickness: self.ok, color: UIColor(white: 230.0 / 255.0, alpha: 1.0))
